/**
 * @description [emby調用第三方播放器播放 支持：Infuse、nPlayer、VLC 、IINA、Movist Pro]
 */
...
if(requestURL.indexOf(addLink) != -1){  // 添加外部播放器連結
  ...
  if(obj.ExternalUrls){ // 兼容終點站的原生 url schema 連結
...
}else if(requestURL.indexOf(embyPlguin) != -1){  // 打開外部播放器
...
}else if(requestURL.indexOf('/Videos/') != -1 && (requestURL.indexOf('/stream/') != -1 || requestURL.indexOf('/Subtitles/') != -1) ){  // 資源路徑偽靜態
  let query = getQueryVariable(requestURL);
